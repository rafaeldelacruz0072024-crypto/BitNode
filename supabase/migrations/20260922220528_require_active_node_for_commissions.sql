-- Golden rule: a beneficiary without an active node cannot receive direct or
-- binary commissions. Binary volume is still propagated through the tree, but
-- any match created while the beneficiary is inactive is consumed without a
-- payout so it cannot be collected retroactively after buying a node.

create schema if not exists bitnode_private;
revoke all on schema bitnode_private from public, anon, authenticated;

create or replace function bitnode_private.has_active_node(p_user_id uuid)
returns boolean
language sql
stable
security definer
...
as $$
  select exists (
    ...
    where contracts.user_id = p_user_id
      and contracts.status = 'active'
      and plans.active
  );
$$;

revoke all on function bitnode_private.has_active_node(uuid)
  from public, anon, authenticated;

create or replace function bitnode_private.require_active_node_for_commission()
returns trigger
language plpgsql
security definer
set search_path = ''
as $$
begin
  if new.commission_type in ('direct', 'binary')
     and not bitnode_private.has_active_node(new.beneficiary_id) then
    return null;
  end if;
...
revoke all on function bitnode_private.require_active_node_for_commission()
  from public, anon, authenticated;

drop trigger if exists a_require_active_node_for_commission
  on public.commission_ledger;
create trigger a_require_active_node_for_commission
before insert on public.commission_ledger
for each row execute function bitnode_private.require_active_node_for_commission();
...
begin
  ...
  if v_sponsor_id is not null
     and bitnode_private.has_active_node(v_sponsor_id) then
  ...
  while v_current is not null and v_step < 100 loop
    ...
    if v_delta > 0 and bitnode_private.has_active_node(v_current) then
    ...
    -- Advancing matched_volume also when inactive forfeits that match and
    -- prevents an old inactive-period match from being paid later.
...
revoke all on function public.process_contract_commissions(text,text,uuid,numeric,text)
  from public, anon, authenticated;
grant execute on function public.process_contract_commissions(text,text,uuid,numeric,text)
  to service_role;

comment on function bitnode_private.has_active_node(uuid) is
  'True only while the member owns at least one active contract on an active node plan.';
comment on function bitnode_private.require_active_node_for_commission() is
  'Drops direct and binary ledger inserts for beneficiaries without an active node.';
