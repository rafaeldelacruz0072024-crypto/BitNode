...
function adminClient() {
  ...
  return (0, import_supabase_js.createClient)(supabaseUrl, serviceRoleKey, { auth: { persistSession: false, autoRefreshToken: false } });
}
function origin(req) {
...
function validIpnSignature(body, signature) {
  ...
  const digest = import_node_crypto.default.createHmac("sha512", secret).update(JSON.stringify(sortObject(body))).digest("hex");
  const expected = Buffer.from(digest, "utf8");
  const received = Buffer.from(signature, "utf8");
  return expected.length === received.length && import_node_crypto.default.timingSafeEqual(expected, received);
}
function registerNowPaymentsRoutes(app2) {
  app2.post("/api/payments/nowpayments/invoice", async (req, res) => {
    try {
      ...
      const transactionId = `NP-${import_node_crypto.default.randomUUID()}`;
...
function admin() {
  ...
  return url && key ? (0, import_supabase_js2.createClient)(url, key, { auth: { persistSession: false, autoRefreshToken: false } }) : null;
}
function token(req) {
  const value = req.header("authorization") || "";
  return value.startsWith("Bearer ") ? value.slice(7) : null;
...
function registerWithdrawalRoutes(app2) {
  app2.post("/api/withdrawals/request", async (req, res) => {
    ...
    const id = `WDR-${import_node_crypto2.default.randomUUID()}`;
...
function adminClient2() {
  ...
  return url && serviceRoleKey2 ? (0, import_supabase_js3.createClient)(url, serviceRoleKey2, {
...
function admin2() {
  const url = process.env.VITE_SUPABASE_URL;
  const key = process.env.SUPABASE_SERVICE_ROLE_KEY;
  return url && key ? (0, import_supabase_js4.createClient)(url, key, { auth: { persistSession: false, autoRefreshToken: false } }) : null;
}
function token2(req) {
  const value = req.header("authorization") || "";
  return value.startsWith("Bearer ") ? value.slice(7) : null;
...
function registerDepositRoutes(app2) {
  app2.post("/api/deposits/request", async (req, res) => {
    ...
    const id = `DEP-${import_node_crypto3.default.randomUUID()}`;
...
function createApiRateLimiter(overrides = {}) {
  return (0, import_express_rate_limit.default)({ windowMs: 15 * 60 * 1e3, limit: 300, standardHeaders: "draft-8", legacyHeaders: false, message, ...overrides });
}
function createFinancialRateLimiter(overrides = {}) {
  return (0, import_express_rate_limit.default)({ windowMs: 60 * 1e3, limit: 30, standardHeaders: "draft-8", legacyHeaders: false, message: { error: "L\xEDmite de operaciones excedido; intenta m\xE1s tarde." }, ...overrides });
}
...
function createApp() {
  ...
  app2.use(import_express.default.json({ limit: "1mb" }));
  app2.use(import_express.default.urlencoded({ limit: "64kb", extended: true }));
