...
export default function AuthPage() {
  ...
  const referral = useMemo(() => {
    const match = window.location.pathname.match(/^\/r\/([^/]+)\/(izquierda|derecha)\/?$/i);
  ...
  const [mode, setMode] = useState<"login" | "signup">(referral ? "signup" : "login");
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(async ({ data }) => {
      ...
      if (!session) return;
      try {
        await applyReferral(session.user.id, String(session.user.user_metadata?.username || username));
        navigate("/dashboard/network");
      } catch (cause) {
        setError(cause instanceof Error ? cause.message : "No se pudo registrar la posición binaria.");
      }
    });
  }, []);
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (mode === "signup" && !result.data.session) {
      setMessage(`Cuenta creada. Confirma tu correo para completar la posición ${referral?.side || "binaria"}.`);
      return;
    }
