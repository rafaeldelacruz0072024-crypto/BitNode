...
import { fetchTransactions, mergeTransactions, persistTransaction } from "@/lib/supabaseAdapter";
...
const catalog = [{ name: "Nodo Diario", rate: "1% – 1.5%", duration: "Indefinida", min: 10 }, { name: "Nodo 7 Días", rate: "2% – 3%", duration: "7 días + capital de vuelta", min: 10 }, { name: "Nodo 14 Días", rate: "3% – 4%", duration: "14 días + capital de vuelta", min: 10 }, { name: "Nodo 21 Días", rate: "4% – 5%", duration: "21 días + capital de vuelta", min: 10 }];
...
export default function Dashboard() {
  ...
  const mutateDeposit = async (amount: number) => { if (!amount || amount < 10) return showNotice("El depósito mínimo local es de $10.00."); const movement = { id: newId("DEP"), type: "deposit" as const, label: "Depósito local", amount, status: "completed" as const, date: new Date().toISOString() }; setUser((prev) => ({ ...prev, balance: prev.balance + amount, movements: [movement, ...prev.movements] })); try { const destination = await persistTransaction({ ...movement, userId: authUserId ?? "", username: user.username }); showNotice(`${money(amount)} acreditados ${destination === "remote" ? "y guardados en Supabase" : "localmente"}.`); } catch { showNotice("Depósito local guardado; Supabase no está disponible."); } };
...
function SectionPanel({ section, user, showNotice, deposit, withdraw, activate }: { section: string; user: LocalUserState; showNotice: (message: string) => void; deposit: (amount: number) => void; withdraw: (amount: number, network: string, wallet: string, fee: number) => void; activate: (item: typeof catalog[number]) => void }) {
