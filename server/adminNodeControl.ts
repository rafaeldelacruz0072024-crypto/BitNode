...
export function classifyNodeTaskCycles(contracts: ContractRow[], cycleRows: CycleRow[], nowMs = Date.now()) {
  ...
  const pendingReset = contracts.filter(contract => {
    ...
    return Boolean(
      cycle?.window_started_at
      && cycle.deadline_at
      && new Date(cycle.deadline_at).getTime() <= nowMs
      && (cycle.completed_tasks?.length || 0) < 4
    );
  });
  const complying = contracts.filter(contract => {
    ...
    return Boolean(
      ...
      && cycle.deadline_at
      && new Date(cycle.deadline_at).getTime() > nowMs
      && (cycle.completed_tasks?.length || 0) > 0
    );
  });
...
export function registerAdminNodeControlRoutes(app: Express) {
  app.get("/api/admin/node-control", async (req, res) => {
    ...
    const [contractsResult, cyclesResult, resetsResult, profilesResult, plansResult] = await Promise.all([
      ...
      admin.client.from("node_task_reset_log").select("id,user_id,contract_id,reason,reset_at,cycle_day_before,completed_tasks_before").gte("reset_at", weekStartedAt).order("reset_at", { ascending: false }).limit(500),
    ...
    const classified = classifyNodeTaskCycles(contracts, cyclesResult.data as CycleRow[] || []);
